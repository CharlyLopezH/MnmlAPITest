...
namespace MinimalAPIPeliculas.Endpoints
{
    public static class PeliculasEndpoints
    {
        ...
        {
            //Creación de enlaces a rutas
            group.MapPost("/", Crear).DisableAntiforgery().AddEndpointFilter<FiltroValidaciones<CrearPeliculaDTO>>();
            group.MapGet("/", Obtener).CacheOutput(c => c.Expire(TimeSpan.FromSeconds(60)).Tag("peliculas-get-cache"));
    ...
    IOutputCacheStore outputCacheStore, IMapper mapper)
        {
            ...
            await outputCacheStore.EvictByTagAsync("peliculas-get-cache", default);
        ...
        IMapper mapper, int pagina = 1, int recordsPorPagina = 10)
        {
            ...
            var peliculas = await repositorio.ObtenerTodas(paginacion);
